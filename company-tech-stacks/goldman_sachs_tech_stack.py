...
from diagrams.programming.language import Python, JavaScript, Java, Go, Scala
...
def create_goldman_sachs_tech_stack_diagram(filename: str, outformat: str) -> None:
    with Diagram("Goldman Sachs 技术栈架构", filename=filename, show=False, outformat=outformat, direction="TB"):
        ...
        with Cluster("客户端层"):
            ...
            with Cluster("交易平台"):
                marquee = Server("Marquee")
                gs_quant = Server("GS Quant")
                clearfact = Server("ClearFact")
                gir = Server("GIR")
                
            with Cluster("移动应用"):
                gs_mobile = Server("GS Mobile")
                marquee_mobile = Server("Marquee Mobile")
                trading_mobile = Server("Trading Mobile")
                
            with Cluster("Web 应用"):
                gs_web = Server("GS Web Portal")
        ...
        with Cluster("开发工具层"):
            ...
            with Cluster("CI/CD"):
                ...
                aws_codedeploy = Server("AWS CodeDeploy")
        ...
        clearfact >> d3_js
        gir >> angular
        gs_mobile >> react_native

        # 前端技术到后端技术
        react >> spring_boot
        typescript >> spring_framework
        d3_js >> django
        angular >> express
        ...
        secdb >> Edge(label="交易系统") >> trading_engine
        oauth >> Edge(label="身份认证") >> kyc
        jenkins >> Edge(label="CI/CD") >> aws_codedeploy
...
if __name__ == "__main__":
    create_goldman_sachs_tech_stack_diagram(filename="goldman_sachs_tech_stack", outformat="png")
    create_goldman_sachs_tech_stack_diagram(filename="goldman_sachs_tech_stack", outformat="pdf")
